...
const sidePath = path.join(projectRoot, "tests", "selenium", "login-blackbox.side");
...
async function main() {
  ...
  try {
    ...
    console.log(`\nSemua skenario selesai. Browser ditahan ${holdMs / 1000} detik.`);
    await sleep(holdMs);
  } catch (error) {
    console.error(`\nTest gagal: ${error.message}`);
    console.error(`Browser ditahan ${holdMs / 1000} detik untuk inspeksi.`);
    await sleep(holdMs);
    process.exitCode = 1;
  } finally {
    await driver.quit();
