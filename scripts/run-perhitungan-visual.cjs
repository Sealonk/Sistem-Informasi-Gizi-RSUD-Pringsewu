...
const sidePath = path.join(projectRoot, "tests", "selenium", "perhitungan-blackbox.side");
...
async function main() {
  ...
  // Get valid token dari API login
  let validToken = null;
  try {
  ...
  for (const test of project.tests) {
    ...
    try {
      for (const command of test.commands) {
        // Replace dummy token dengan valid token
        let processedCommand = { ...command };
        if (command.command === "executeScript" && 
            command.target.includes("localStorage.setItem('token'")) {
          processedCommand.target = `window.localStorage.setItem('token', '${validToken}');`;
        }

        const label = [processedCommand.command, processedCommand.comment || processedCommand.target]
          .filter(Boolean)
          .join(" - ");

        console.log(`  ${label}`);
        await runCommand(driver, processedCommand, variables);
        await sleep(delayMs);
      }
    } catch (error) {
      testFailed = true;
