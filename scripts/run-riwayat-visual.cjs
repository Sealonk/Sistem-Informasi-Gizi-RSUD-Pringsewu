...
const sidePath = path.join(projectRoot, "tests", "selenium", "riwayat-blackbox.side");
...
async function runCommand(driver, command, variables) {
  ...
  switch (command.command) {
    ...
    case "chooseOkOnNextConfirmation":
      await driver.executeScript("window.confirm = function(msg) { window.__lastConfirmMsg = msg; return true; };");
      break;

    case "chooseCancelOnNextConfirmation":
      await driver.executeScript("window.confirm = function(msg) { window.__lastConfirmMsg = msg; return false; };");
      break;

    case "assertConfirmation": {
      const actualMsg = await driver.executeScript("return window.__lastConfirmMsg;");
      if (actualMsg !== target) {
        throw new Error(`Assert confirmation gagal: "${actualMsg}" tidak sama dengan "${target}"`);
      }
      break;
    }
