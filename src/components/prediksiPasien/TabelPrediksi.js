export default function TabelPrediksi({ data = [] }) {
  return (
    <section className="prediction-table rounded-2xl border border-slate-200 bg-white p-4 sm:p-6">
      ...
      <div className="mt-4 max-h-[420px] overflow-auto rounded-xl border border-slate-100">
        <table className="w-full border-collapse text-sm">
          ...
          <tbody>{data.map((item) => <tr key={item.tanggal} className="border-t border-slate-100 even:bg-slate-50/50 hover:bg-violet-50/70 transition-colors duration-200 motion-reduce:transition-none">
            ...
            <td className="px-4 py-3 text-right font-semibold text-slate-700 tabular-nums">{item.jumlah.toLocaleString("id-ID")}</td>
          </tr>)}</tbody>
        </table>
      </div>
    </section>
